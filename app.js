...
const videoDisplay = document.getElementById("videoContainer");
...
function displayMenuItems(menuItems) {
  const menuCards = menuItems
    ...
    .join("");
  cardContainer.innerHTML = menuCards;
}
...
filterButtons.forEach((link) => {
  link.addEventListener("click", (e) => {
    ...
    const filteredMenu =
      filterValue === "all"
        ? menu
        : menu.filter((item) => item.category === filterValue);
...
video.src = "https://files.codingninjas.in/restaurant-1706081051.mp4";
video.autoplay = true;
video.loop = true;
video.muted = true;
